const SITE_IMAGES = [
  ...
  "source/image/8/맨슨과사탄.jpg",
  "source/image/9/디제이의음모론.jpg",
  "source/image/9/바틀즈불.png",
  "source/image/9/주다스프리스트.jpg",
  ...
  "source/image/13/Léo_Taxil_(Melandri).jpg",
  "source/image/13/레오탁실저서.jpg",
  "source/image/14/몰렉.jpg",
  "source/image/14/제목없음-1.jpg",
  "source/image/14/Milton_paradise.jpg",
  "source/image/14/men-eating-at-communal-table-at-bohemian-club-grove-by-arnold-genthe.jpg.webp",
  "source/image/15/Pentagram_with_one_point_down_(de_Guaita).jpg",
  "source/image/15/기사단.jpg",
  "source/image/15/손동작.png",
...
function startFlicker(group) {
  ...
  function tick() {
    ...
    if (current && current !== next) current.style.opacity = "0";
...
document.addEventListener("visibilitychange", () => {
  flickers.forEach((flicker) => {
    if (document.hidden) flicker.stop();
    else flicker.start();
  });
});
